  ...
  createConnection (options, callback) {
    ...
    if (options.timeouts.connection) {
      connectionTimeout = setTimeout(() => {
        return callback(new ConnectionTimeoutError())
      ...
      const onResponse = (res) => {
        transferTimeout = setTimeout(() => {
          if (res.listenerCount('error') > 0) {
            res.emit('error', new TransferTimeoutError())
          }
          res.destroy()
          // res.socket.destroy(new TransferTimeoutError())
          // console.error(res.socket === request.socket)
          // res.socket.destroy(new TransferTimeoutError())
          // process.nextTick(() => res.socket.emit('error', new TransferTimeoutError()))
          // if (res.listenerCount('error') > 0) {
          //   res.emit('error', new TransferTimeoutError())
          // }
          // request.emit('error', new TransferTimeoutError())
          // res.emit('error', new TransferTimeoutError())
          // defer actual destruction
          // setTimeout(() => res.destroy())
