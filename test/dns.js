...
t.test('supports no options passed', async (t) => {
  ...
  const fakeLookup = (hostname, options, callback) => {
    lookupCalled += 1
    t.hasStrict(options, dns.defaultOptions, 'applied default options')
...
t.test('supports family passed directly as options', async (t) => {
  ...
  const fakeLookup = (hostname, options, callback) => {
    lookupCalled += 1
    t.hasStrict(options, { ...dns.defaultOptions, family: 4 }, 'kept family setting')
    process.nextTick(callback, null, '127.0.0.1', 4)
  }
  const lookup = dns.getLookup({ ...DEFAULT_OPTS, lookup: fakeLookup })
...
t.test('varies when options change', async (t) => {
  ...
  const fakeLookup = (hostname, options, callback) => {
    ...
    if (lookupCalled === 1) {
      ...
      t.hasStrict(options, { ...dns.defaultOptions, family: 6 }, 'kept family from second lookup')
...
t.test('lookup can return all results', async (t) => {
  ...
  const fakeLookup = (hostname, options, callback) => {
    lookupCalled += 1
    t.hasStrict(options, { ...dns.defaultOptions, all: true }, 'applied default options')
